                            ...
                            child: Row(
                              ...
                              children: [
                                Container(
                                  ...
                                  child: Image.network(
                                    "${ConstantUrl.uploadUrl}${widget._modelExerciseDetail.image == '' ? widget._modelExerciseDetail.thumbnailUrl : widget._modelExerciseDetail.image}",
                                    width: double.infinity,
                                    height: double.infinity,
                                    fit: BoxFit.contain,
...
class _WidgetDetailData extends State<WidgetDetailData>
  ...
  void initState() {
    ...
    if (widget._modelExerciseList.videoUrl != null &&
      ...
      // _controller.play();
      isVideo = true;
    } else if (widget._modelExerciseList.hostVideoUrl != null &&
        widget._modelExerciseList.hostVideoUrl!.isNotEmpty) {
      _controller = VideoPlayerController.network(
        widget._modelExerciseList.hostVideoUrl!,
      );
  ...
  Widget build(BuildContext context) {
    ...
    final deviceRatio = 1.8 * size.width / size.height;
              ...
              Expanded(
                ...
                child: Container(
                  child: Stack(
                    children: [
                      Center(
                        child: !(widget._modelExerciseList.videoUrl != null &&
                                widget._modelExerciseList.videoUrl!.isNotEmpty)
                            ? ((widget._modelExerciseList.hostVideoUrl !=
                                        null &&
                                    widget._modelExerciseList.hostVideoUrl!
  ...
  void pauseTimer() {
    if (_timer != null) {
      _timer!.cancel();
      _timer = null;
      if (isVideo && _controller.value.isInitialized) _controller.pause();
    } else {
      startTimer();
      if (isVideo && _controller.value.isInitialized) _controller.play();
    }
    setState(() {});
  }
}
...
const String url =
    "http://10.10.15.46:8000/uploads/exercise_video_1676097605.mp4";
                ...
                : Container(
                    ...
                    child: FutureBuilder(
                      ...
                      builder: (context, snapshot) {
                        widget._modelDummySend.type == CHALLENGES_WORKOUT
                            ? addDayComplete(
                                context, widget._modelDummySend.id, "")
                            : null;
                        addWholeHistory(
                            context,
                            widget.totalCal,
